...
func (c *creditCard) processPayment(totalAmount float64) string {
	return fmt.Sprintf("Processed credit card payment of $%.2f", totalAmount)
}

func (c *creditCard) calculateFee(amount float64) float64 {
	return c.feePercentage * amount
}
...
func (p *paypal) processPayment(totalAmount float64) string {
	return fmt.Sprintf("Processed PayPal payment of $%.2f", totalAmount)
}
...
func completePayment(p paymentMethod, amount float64) {
	...
	fmt.Printf("Subtotal: $%.2f, Total: $%.2f\n", amount, totalAmount)
	fmt.Println(p.processPayment(totalAmount))
}
